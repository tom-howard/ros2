...
6. Then, we combine the `left` and `right` data arrays, flip them so that the data is arranged from left to right, and then convert this to a `numpy` array (see the figure below).
